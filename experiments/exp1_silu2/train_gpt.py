...
try:
    ...
    try:
        ...
        def _fa_impl(q, k, v, causal=True):
            # q: (B, T, nh_q, hd), k/v: (B, T, nh_kv, hd)
            nh_q, nh_kv = q.shape[2], k.shape[2]
            q2 = q.transpose(1, 2)   # (B, nh_q, T, hd)
            k2 = k.transpose(1, 2)   # (B, nh_kv, T, hd)
            v2 = v.transpose(1, 2)   # (B, nh_kv, T, hd)
            if nh_q != nh_kv:
                groups = nh_q // nh_kv
...
def main() -> None:
    ...
    # Smoke-test fallback: if no flash_attn package is installed, re-enable math SDPA
    try:
