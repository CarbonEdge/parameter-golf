...
try:
    from flash_attn_interface import flash_attn_func as _fa_impl
except ImportError:
    try:
        from flash_attn import flash_attn_func as _fa_impl
    except ImportError:
        # SDPA fallback — slower but works on any CUDA GPU (e.g. RTX 4080 for smoke tests)
        # Handles GQA: expand k/v heads to match q heads before calling SDPA
        def _fa_impl(q, k, v, causal=True):
            nh_q, nh_kv = q.shape[2], k.shape[2]
            q2 = q.transpose(1, 2)
            k2 = k.transpose(1, 2)
            v2 = v.transpose(1, 2)
            if nh_q != nh_kv:
...
def flash_attn_3_func(q, k, v, causal=True):
    return _fa_impl(q, k, v, causal=causal)
class Hyperparameters:
    # NOTE: sp4096 is not yet available in the challenge manifest (as of 2026-03-29).
    # This experiment now uses sp1024 with 4096-token training sequences.
    # To use 4096 BPE vocab when available: set DATA_PATH/TOKENIZER_PATH/VOCAB_SIZE env vars.
    ...
    # exp4_4kseqlen: 4096-token sequence length (was 2048 in SOTA).
    # Longer context allows model to learn longer-range dependencies.
    # "4k seq length" baseline at 1.2014 bpb was never tried with the full 11L SOTA stack.
...
def main() -> None:
    ...
    try:
        from flash_attn_interface import flash_attn_func as _check  # noqa: F401
    except ImportError:
        try:
            from flash_attn import flash_attn_func as _check  # noqa: F401
        except ImportError:
